...
class ECAAttention(nn.Module):
    ...
    def __init__(self, kernel_size=3):
        super().__init__()
        self.gap = nn.AdaptiveAvgPool1d(1)  # 定义全局平均池化层，将空间维度压缩为1x1
    ...
    def forward(self, x):
        ...
        y = y.permute(0, 2, 1)  # 移除最后一个维度并转置，为1D卷积准备，变为bs,1,c
        #print(y.shape)
        y = self.conv(y)  # 对转置后的y应用1D卷积，得到bs,1,c维度的输出
        y = self.sigmoid(y)  # 应用Sigmoid函数激活，得到最终的注意力权重
        y = y.permute(0, 2, 1)  # 再次转置并增加一个维度，以匹配原始输入x的维度
...
class TFEDCN(torch.nn.Module):
    ...
    def forward(self, inputs, is_training=True, **args):
        ...
        fused = self.eca(proj_mu, proj_h)
        #print(fused.shape)
        fused = nn.Linear(512, 2048).cuda()(fused)  # [10, 8, 2048]
